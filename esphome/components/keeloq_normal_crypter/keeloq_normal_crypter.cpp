...
bool KeeloqNormalCrypter::decrypt(remote_base::KeeloqData &data){
    return this->decrypt(data, this->normal_keygen(data.address));
}

bool KeeloqNormalCrypter::encrypt(remote_base::KeeloqData &data){
    if(!mkey_)
        return false;
    this->encrypt(data, this->normal_keygen(data.address));
    return true;
}
...
uint64_t KeeloqNormalCrypter::normal_keygen(uint32_t address) {
  static uint64_t key;
  ...
  key = keeloq_decrypt(address | 0x60000000, mkey_);
  key = key << 32 | keeloq_decrypt(address | 0x20000000, mkey_);
...
bool KeeloqNormalCrypter::decrypt(remote_base::KeeloqData &data, uint64_t key) {
  ...
  if ((chk & 0x3FF) != (data.address & 0x3FF)) {
    ESP_LOGD(TAG, "Failed to verify discrimination bits, expected %04X, got %04X", data.address & 0x3FF, chk & 0x3FF);
    return false;
  }

  // verify button code
  if (((chk >> 12) & 0x0F) != data.command) {
    ESP_LOGD(TAG, "Failed to verify command, expected %04X, got %04X", data.command, (chk >> 12) & 0x0F);
    return false;
  }

  data.encrypted = clear;
  ESP_LOGV(TAG, "Decrypted: Address %07X Sync: %04X", data.address, sync);
  return true;
}

void KeeloqNormalCrypter::encrypt(remote_base::KeeloqData &data, uint64_t key) {
  // uint16_t chk = ((data.command & 0xF) << 12) | (data.address & 0x3FF);
  // uint32_t clear = (chk << 16) | data.sync;
  uint32_t clear = data.encrypted;
  data.encrypted = keeloq_encrypt(clear, key);

  ESP_LOGD(TAG, "Encrypt: address %07X Clear: %08X Encrypted: %08X", data.address, clear, data.encrypted);
}
